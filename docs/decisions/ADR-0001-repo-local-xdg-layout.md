...
`changes` operates inside arbitrary Git repositories. Its configuration, durable release metadata, and templates need to be reviewable alongside the repository content they affect.
...
Treat `.local/state/changes/**` as transient local state that should be ignored by Git.
...
- transient state remains clearly separated from durable metadata
